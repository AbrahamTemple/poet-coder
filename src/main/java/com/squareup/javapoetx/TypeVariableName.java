...
public final class TypeVariableName extends com.squareup.javapoetx.TypeName {
  public final String name;
  public final List<com.squareup.javapoetx.TypeName> bounds;

  private TypeVariableName(String name, List<com.squareup.javapoetx.TypeName> bounds) {
    this(name, bounds, new ArrayList<>());
  }

  private TypeVariableName(String name, List<com.squareup.javapoetx.TypeName> bounds, List<com.squareup.javapoetx.AnnotationSpec> annotations) {
    super(annotations);
    this.name = checkNotNull(name, "name == null");
    this.bounds = bounds;

    for (com.squareup.javapoetx.TypeName bound : this.bounds) {
  ...
  @Override public com.squareup.javapoetx.TypeName withoutAnnotations() {
    return new TypeVariableName(name, bounds);
  }

  public TypeVariableName withBounds(Type... bounds) {
    return withBounds(com.squareup.javapoetx.TypeName.list(bounds));
  }

  public TypeVariableName withBounds(com.squareup.javapoetx.TypeName... bounds) {
    return withBounds(Arrays.asList(bounds));
  }

  public TypeVariableName withBounds(List<? extends com.squareup.javapoetx.TypeName> bounds) {
    ArrayList<com.squareup.javapoetx.TypeName> newBounds = new ArrayList<>();
    newBounds.addAll(this.bounds);
    newBounds.addAll(bounds);
    return new TypeVariableName(name, newBounds, annotations);
  }

  private static TypeVariableName of(String name, List<com.squareup.javapoetx.TypeName> bounds) {
    // Strip java.lang.Object from bounds if it is present.
    List<com.squareup.javapoetx.TypeName> boundsNoObject = new ArrayList<>(bounds);
  ...
  public static TypeVariableName get(String name, com.squareup.javapoetx.TypeName... bounds) {
    return TypeVariableName.of(name, Arrays.asList(bounds));
  }

  /** Returns type variable named {@code name} with {@code bounds}. */
  public static TypeVariableName get(String name, Type... bounds) {
    return TypeVariableName.of(name, com.squareup.javapoetx.TypeName.list(bounds));
  }
  ...
  static TypeVariableName get(
    ...
    if (typeVariableName == null) {
      ...
      List<com.squareup.javapoetx.TypeName> bounds = new ArrayList<>();
      List<com.squareup.javapoetx.TypeName> visibleBounds = Collections.unmodifiableList(bounds);
      typeVariableName = new TypeVariableName(element.getSimpleName().toString(), visibleBounds);
      typeVariables.put(element, typeVariableName);
      for (TypeMirror typeMirror : element.getBounds()) {
        bounds.add(com.squareup.javapoetx.TypeName.get(typeMirror, typeVariables));
      }
  ...
  public static TypeVariableName get(TypeParameterElement element) {
    ...
    List<com.squareup.javapoetx.TypeName> boundsTypeNames = new ArrayList<>();
    for (TypeMirror typeMirror : boundsMirrors) {
      boundsTypeNames.add(com.squareup.javapoetx.TypeName.get(typeMirror));
    }
